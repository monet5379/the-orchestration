...
| 버전 | Step | 요약 |
|---|---|---|
| **0.1.30** | 6 | ADJUST 15초 · 상대 버튼 안내 · CPU 최종행동 1회 · 런처/캐시 안정화 |
| 0.1.29 | 6 | ADJUST 한 번 확정 · 노란 테두리 · 양측 완료 시 즉시 RESOLVE |
...
### 0.1.30 (2026-08-30) — ADJUST 15초 · 버튼 안내 · 런처

- **ADJUST** 30초 → **15초** (시간 팽창 +2s → 17초)
- `opponentButtonHint`: 기본 유지/바꾸기 안내 · 페이크는 바꾸기 위장 · 본능/×가면은 페이크까지 정확
- CPU `planCpuAdjustAction`: 유지|바꾸기|페이크 **한 번에 결정** 후 커밋 (안내 덮어쓰기 제거)
- 타이틀 새 게임: 인라인 확인 · 메뉴 `phase: null` · SELECT 부트 보장
- `serve.py` (JS no-store) · `run.ps1` · `run.bat` 위임 · import `?v=`

### 0.1.29 (2026-08-29) — ADJUST 한 번 확정 · 조기 종료

- 유지·변경·페이크 **한 번이면 끝** (`playerAdjusted`: kept / changed / bluffed)
- 확정 후 가위·바위·보·유지·페이크 전부 잠금 · 유효 패에 **노란 테두리** (`is-committed`)
- 유지/페이크는 패 표시가 같으므로 해당 액션 버튼에도 강조
- `playerAdjusted` + `cpuAdjusted` 완료 시 타이머 중단 → **즉시 `ADVANCE_TO_RESOLVE`**
...
- [ ] 세이브 없음 → 새 게임으로 매치 진입 · **SELECT 타이머 표시** (`tests/smoke-menu.mjs`)
- [ ] ADJUST: **15초** 타이머 (시간 팽창 **17초**)
- [ ] 본능·가면 없음: CPU 유지 → `유지 버튼` / 변경·페이크 → `바꾸기 버튼` (안내 **1회·불변**)
- [ ] 본능 또는 ×가면: 페이크 시 `페이크 버튼을 눌렀습니다` 정확 표시
- [ ] ADJUST: 유지/페이크/변경 한 번 확정 · 노란 테두리 · 양측 완료 시 즉시 RESOLVE
...
- [ ] `run.bat` → Console에 `v0.1.29` ready 로그
...
- [ ] ADJUST: 30초 타이머 (시간 팽창 32초) · 미결정 시 만료 후 RESOLVE
- [ ] ADJUST: 내가 수정·페이크 → **소리·빨간 플래시 없음**
- [ ] ADJUST: 상대 페이크 → 페이크음 + 렌즈 플래시
- [ ] SELECT: 60초 타이머 · 패 선택 시 금속음·플래시
...
- 결과: (수동 기입)
- 메모: ADJUST 15s · 버튼 안내 · CPU 최종 1회 · 인라인 새 게임 · 런처 정리

### 2026-08-29 · v0.1.29

- 환경: Windows / Chrome · `run.bat`
- 결과: (수동 기입)
- 메모: ADJUST 한 번 확정 · 노란 테두리 · 양측 완료 시 즉시 RESOLVE
