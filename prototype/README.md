...
**현재 버전: v0.1.29** (Step 0~6 완료)

버전별 변경·수동 테스트 이력 → [`ARCHIVE.md`](./ARCHIVE.md)  
공유용 패치노트 → [`../docs/patch-note/0.1.29.md`](../docs/patch-note/0.1.29.md)

---

## 지금 할 수 있는 것

- 가위·바위·보 + **60초** 패 선택 · **30초** **ADJUST**(유지 / 변경 / 페이크 · **한 번 확정**)
...
|---|---|
| 버튼 순서 | 가위 → 바위 → 보 |
| ADJUST | 최대 30초(시간 팽창 +2초) · `[유지]` / 다른 패로 변경 / `[페이크]` · **한 번이면 끝** |
...
│   ├── ui/       # button-panel, countdown, overlay …
│   ├── scenes/   # match, menu, cell
│   └── ai/       # opponent
└── tests/
```
